# CHARACTER CREATION

Creating a character involves a series of meaningful choices that define who your character is, what they can do, and why they adventure.

## Step-by-Step Process

### Step 1: Choose Your Attributes

Select your starting attribute array and assign values to the six attributes.

### Step 2: Choose Your Species

Select one of the four species, applying your attribute bonus and gaining species traits.

### Step 3: Choose Your Archetype

Pick one of seven archetypes that determines your starting skills and equipment.

### Step 4: Choose Your Calling

Select one of nine callings that defines your inner motivation and provides special abilities.

### Step 5: Calculate Derived Values

Determine your Guard, Vitality, and other derived statistics.

### Step 6: Add Final Details

Name your character, develop a brief background, and create connections.

---

## Attributes

Attributes range from -5 to +5, with 0 representing the human average.

### Starting Arrays (choose one):

- **Standard:** +2, +1, +1, 0, 0, -1
- **Specialized:** +3, +2, 0, 0, -1, -2
- **Balanced:** +1, +1, +1, 0, 0, 0

### The Six Attributes:

- **Might** - Physical force, strength, and endurance
- **Grace** - Precision, agility, and coordination
- **Mind** - Reasoning, intellect, and knowledge
- **Awareness** - Perception, intuition, and attunement
- **Will** - Fortitude, determination, and spiritual power
- **Presence** - Charisma, magnetism, and social influence

---

## Species

Your species provides an attribute bonus and unique traits that integrate with the Momentum system.

### DWARF

_Sturdy mountain folk known for resilience and craftsmanship_

**Attribute Bonus:** +1 Might or +1 Will (your choice)

**Species Traits:**

- **Stone Endurance:** Once per scene, reduce damage from one attack by 2 (stacks with armor)
- **Craftwise:** When examining worked materials, ask one question the GM must answer truthfully
- **Steady Stance:** Switching from Defensive stance costs no Momentum (stubborn immovability)

**Common Callings:** Sentinel, Guardian, Advocate
**Natural Archetypes:** Fighter, Paladin, Cleric

### ELF

_Graceful people attuned to magic and nature_

**Attribute Bonus:** +1 Grace or +1 Awareness (your choice)

**Species Traits:**

- **Fey Grace:** When you roll 11+ on any check, gain +1 Momentum (once per round)
- **Magical Affinity:** Can sense magic within Near range without Sorcery skill (but need skill for details)
- **Elven Focus:** When casting spells, ignore the first point of armor penalty

**Common Callings:** Scholar, Seeker, Mediator
**Natural Archetypes:** Wizard, Ranger, Bard

### HALFLING

_Small, nimble folk with remarkable luck and courage_

**Attribute Bonus:** +1 Will or +1 Presence (your choice)

**Species Traits:**

- **Lucky:** Once per scene, reroll any single die (keep new result)
- **Brave Heart:** When an ally goes to negative Momentum, immediately gain +1 Momentum
- **Small but Mighty:** Move through enemy zones without provoking. When successfully Hide, enemies get -2 to detect

**Common Callings:** Enthusiast, Guardian, Mediator
**Natural Archetypes:** Rogue, Bard, Cleric

### HUMAN

_Versatile and ambitious, adapting to any challenge_

**Attribute Bonus:** +1 to any attribute of your choice

**Species Traits:**

- **Versatile:** Start with one extra skill at Novice (-1)
- **Determined:** When at negative Momentum, next successful action gains +1 additional Momentum
- **Adaptable:** During any rest, may swap one Competent skill with another

**Common Callings:** Any - humans show no pattern
**Natural Archetypes:** Any - human flexibility

---

## Archetypes

Your archetype determines your initial skill training, equipment, and combat role.

### FIGHTER

_Master of weapons and combat techniques_

**Key Skills:**

- Combat (Professional +1)
- Athletics (Novice -1)
- Command (Novice -1)

**Starting Equipment:** Medium armor, martial weapon, shield
**Starting Edge:** Choose Weapon Focus (+1 damage with one weapon type) or Tough (+2 Guard)
**Natural Callings:** Champion, Guardian, Sentinel

### RANGER

_Wilderness expert and skilled hunter_

**Key Skills:**

- Combat (Novice -1)
- Survival (Professional +1)
- Investigate (Novice -1)

**Starting Equipment:** Light armor, bow, tracking kit
**Starting Edge:** Choose Fast Movement (+1 zone) or Precise Strike (reroll one damage die)
**Natural Callings:** Seeker, Scholar, Sentinel

### PALADIN

_Divine warrior and inspiring leader_

**Key Skills:**

- Combat (Competent +0)
- Medicine (Novice -1)
- Command (Professional +1)
- Sorcery (Novice -1)

**Starting Equipment:** Heavy armor, weapon, holy symbol
**Special:** Can cast divine spells regardless of attributes
**Natural Callings:** Advocate, Guardian, Champion

### ROGUE

_Skilled infiltrator and versatile expert_

**Key Skills:**

- Stealth (Professional +1)
- Finesse (Professional +1)
- Combat (Competent +0)

**Starting Equipment:** Light armor, daggers, thieves' tools
**Special:** Two Professional skills at start (only archetype with this advantage)
**Natural Callings:** Seeker, Enthusiast, Scholar

### WIZARD

_Master of arcane knowledge and magical theory_

**Key Skills:**

- Sorcery (Professional +1)
- Lore (Professional +1)
- Investigate (Novice -1)

**Starting Equipment:** Robes, spellbook, component pouch
**Natural Callings:** Scholar, Seeker, Advocate
**Special:** Can cast arcane spells regardless of attributes

### CLERIC

_Channel of divine power and spiritual healer_

**Key Skills:**

- Medicine (Professional +1)
- Sorcery (Competent +0)
- Empathy (Professional +1)

**Starting Equipment:** Medium armor, mace, holy symbol
**Natural Callings:** Guardian, Mediator, Advocate
**Special:** Can cast divine spells regardless of attributes

### BARD

_Performer, storyteller, and magical artist_

**Key Skills:**

- Perform (Professional +1)
- Deceive (Novice -1)
- Sorcery (Competent +0)

**Starting Equipment:** Light armor, instrument, rapier
**Natural Callings:** Enthusiast, Mediator, Seeker
**Special:** Can cast performance magic regardless of attributes

_Note: All other skills start at Untrained (-2)_

---

## Callings

Your Calling represents your inner drive and provides Momentum benefits and narrative permissions.

### 1. THE ADVOCATE

_"The world must be made right"_

**Core Drive:** To improve, perfect, and bring order to chaos
**Fear:** Being corrupt, defective, or wrong
**Desire:** To be good, right, accurate, and virtuous

**Momentum Benefit - Truth Revealed:** When you or an ally succeeds at revealing truth or exposing deception (any Investigation, Empathy, or social check that uncovers hidden information), gain +1 Momentum

**Narrative Permission - Truth Sense:** You can always tell when someone is lying about wrongdoing or corruption. No roll needed - you simply know.

### 2. THE GUARDIAN

_"I exist to help others"_

**Core Drive:** To feel needed and appreciated through service
**Fear:** Being unloved or unwanted
**Desire:** To feel loved and needed by others

**Momentum Benefit - Protective Presence:** When an ally within Close range takes damage, you gain +1 Momentum (once per round maximum)

**Narrative Permission - Helper's Intuition:** You always know when someone genuinely needs help, even if they're trying to hide it. No roll needed.

### 3. THE EXEMPLAR

_"Success proves my worth"_

**Core Drive:** To achieve, excel, and be valued for accomplishments
**Fear:** Being worthless or without value apart from achievements
**Desire:** To feel valuable and worthwhile

**Momentum Benefit - Excellence Rewarded:** Whenever you achieve a Critical Success (13+) on any roll, gain +1 additional Momentum beyond the normal critical bonus

**Narrative Permission - Victory Sense:** You can instinctively assess the "win condition" of any situation - what would constitute success here. No roll needed.

### 4. THE SEEKER

_"I must find my true self"_

**Core Drive:** To understand themselves and find their unique identity
**Fear:** Having no identity or significance
**Desire:** To find themselves and their significance

**Momentum Benefit - Novel Approach:** Once per scene, when you attempt something you've never tried before or approach a problem in a completely novel way, gain +3 Momentum regardless of success

**Narrative Permission - Soul Reading:** Once per scene, you can ask one creature a deep personal question that they must answer honestly (though they may be cryptic).

### 5. THE SCHOLAR

_"Knowledge reveals truth"_

**Core Drive:** To understand, comprehend, and master knowledge
**Fear:** Being incompetent, invaded, or overwhelmed
**Desire:** To be capable and understanding

**Momentum Benefit - Perpetual Readiness:** You begin each scene with +1 Momentum instead of 0, representing your perpetual state of mental preparedness

**Narrative Permission - Contingency Planning:** Once per scene, declare you prepared for this exact situation and produce a relevant tool, fact, or minor preparation.

### 6. THE SENTINEL

_"Loyalty and vigilance keep us safe"_

**Core Drive:** To find security and support through loyalty and preparation
**Fear:** Being without support, guidance, or security
**Desire:** To have security and support

**Momentum Benefit - Ever Watchful:** Whenever you or an ally you can see is attacked, gain +1 Momentum (once per round maximum)

**Narrative Permission - Danger Sense:** You cannot be surprised or ambushed. You always act in the first round of combat, even if caught off-guard.

### 7. THE ENTHUSIAST

_"Life is an adventure to experience"_

**Core Drive:** To maintain happiness, freedom, and avoid pain
**Fear:** Being trapped in pain or deprivation
**Desire:** To be satisfied and content, to have needs fulfilled

**Momentum Benefit - Contagious Enthusiasm:** When you gain Momentum from any source, one adjacent ally also gains +1 Momentum (spreading your enthusiasm)

**Narrative Permission - Silver Lining:** You can always find something positive or useful in any situation, no matter how dire. This insight provides actual mechanical or narrative benefit.

### 8. THE CHAMPION

_"Strength protects what matters"_

**Core Drive:** To be self-reliant, in control, and protect others
**Fear:** Being controlled, vulnerable, or weak
**Desire:** To be self-reliant and in control of their destiny

**Momentum Benefit - Protective Wrath:** While any ally is below half Guard or Vitality, you gain +1 Momentum on all successful attacks

**Narrative Permission - Unshakeable Will:** Intimidation attempts against you automatically fail. You project strength that cannot be undermined.

### 9. THE MEDIATOR

_"Harmony brings peace to all"_

**Core Drive:** To maintain inner and outer peace and harmony
**Fear:** Loss of connection, separation, conflict
**Desire:** To have inner and outer peace

**Momentum Benefit - Bridge Builder:** The first time each scene you help resolve any conflict (combat, social, or otherwise), you and all involved parties gain +1 Momentum

**Narrative Permission - Universal Understanding:** You understand the basic meaning and emotional content of any communication, regardless of language barriers.

---

## Derived Values

### Health

- **Guard:** 12 + max(Might, Grace, Will)
  - Ablative defense that absorbs damage first
  - Allows tough, agile, or mentally resilient characters to be effective defenders
  - Recoverable between scenes
  - Can be restored in combat with Rally action (recover 1d6 Guard, once per scene)
  - Some archetypes add bonuses: Divine Grace +4, Arcane Battery +2
- **Vitality:** 10 + Will
  - Core health representing serious injury
  - Damage to Vitality causes lasting consequences
  - At 0 Vitality: Unconscious and dying

### Death & Dying

When reduced to 0 Vitality, you fall unconscious and begin dying.

**Death Saves:** Roll 2d6 + Will at the start of each turn:

- **5 or less:** Lose 1 Vitality (getting worse)
- **6-7:** Stable for this round (holding on)
- **8-10:** Stabilize at 0 Vitality (unconscious but stable)
- **11+:** Regain consciousness at 1 Vitality (heroic recovery)

**Death:** Occurs at -3 Vitality (down from -5)
**Stabilization:** Medicine check (TN 8) or any magical healing

### Momentum

- **Starting Momentum:** 0 (Scholars start at +1)
- **Range:** -3 to +6 (reduced from original -6 to +10)
- **Momentum Gates:** Some abilities require +3 or +6 Momentum to activate

### Magic Access

**Universal Rule:** If your archetype provides the Sorcery skill, you can cast spells regardless of your attributes.

**Alternative Access:** Characters without a spellcasting archetype can learn magic if:

- Mind + Awareness ≥ +2 (Arcane magic)
- Will + Presence ≥ +2 (Divine magic)
- Awareness + Will ≥ +2 (Primal magic)

### Movement & Zones

- **Speed:** One zone per round (standard)
- **Engaged:** Melee combat distance
- **Close:** One move away (throwing range)
- **Far:** Two moves away (bow range)

---

## Character Sheet Organization

### Front Page

- **Character Concept:** Name, species, archetype, calling
- **Attributes:** Six core attributes with modifiers
- **Skills:** All fifteen skills with current ratings
- **Derived Values:** Guard, Vitality, Momentum
- **Combat Stats:** Stance preferences, weapons, armor

### Back Page

- **Species Traits:** Your three species abilities
- **Archetype Abilities:** Skills and special features
- **Calling Benefits:** Momentum benefit and narrative permission
- **Equipment:** Gear, wealth, and possessions
- **Background:** History, connections, and goals

---

## Character Creation Example

**Thornwick Ironfoot**

- **Species:** Dwarf (+1 Will chosen)
- **Attributes:** Specialized Array: Might +2, Grace -2, Mind -1, Awareness 0, Will +3[+1], Presence 0
- **Archetype:** Cleric
  - Medicine (Professional +1)
  - Sorcery (Competent +0)
  - Empathy (Professional +1)
- **Calling:** The Guardian
  - Can spend 1 Momentum to halve damage to dying ally
  - Always knows who needs help
- **Derived Values:**
  - Guard: 15 (12 + Will 3, highest of Might/Grace/Will)
  - Vitality: 13 (10 + Will 3)
  - Magic Access: Yes (Cleric archetype grants divine casting regardless of attributes)

Thornwick combines divine magic with practical medicine to protect his allies. His Guardian calling makes him exceptionally attuned to others' needs, while his dwarven resilience allows him to endure hardship to help others.

---

## Character Creation Quick Tips

### For New Players - Classic Combinations:

**"The Knight"**
Human Fighter Champion

- Simple, effective, heroic

**"The Wise Mage"**  
Elf Wizard Scholar

- Classic spellcaster

**"The Healer"**
Human Cleric Guardian

- Support-focused

**"The Scout"**
Halfling Rogue Enthusiast

- Skill-focused, cheerful

### For Experienced Players - Unexpected Combinations:

**"The Stubborn Innovator"**
Dwarf Wizard Exemplar

- Proves dwarves can master magic

**"The Tiny Avenger"**
Halfling Paladin Advocate

- Small crusader for justice

**"The Eternal Student"**
Elf Fighter Scholar

- Martial arts as academic study

**"The Wandering Soul"**
Human Ranger Seeker

- Finding themselves in the wilderness
