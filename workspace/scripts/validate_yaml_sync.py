...
MARKDOWN_DIRECTORIES = [
    'core_rules',
    'content_modules',
    'content_modules/content_types',
]

# Individual markdown files to check (relative to project root)
MARKDOWN_FILES = [
    # Add specific files here, e.g.:
    # 'quick_reference.md',
]
...
def main():
    ...
    for dir_path in MARKDOWN_DIRECTORIES:
        full_dir = project_root / dir_path
        if full_dir.exists():
            md_files.extend(full_dir.glob('*.md'))

    # Check configured individual files
    ...
    if not md_files:
        ...
        print(f"Configure MARKDOWN_DIRECTORIES or MARKDOWN_FILES in {__file__}")
        sys.exit(0)
